...
local Converted = {
	["_RCL.lua"] = Instance.new("ScreenGui");
...
Converted["_RCL.lua"].IgnoreGuiInset = true
Converted["_RCL.lua"].ScreenInsets = Enum.ScreenInsets.DeviceSafeInsets
Converted["_RCL.lua"].ZIndexBehavior = Enum.ZIndexBehavior.Sibling
Converted["_RCL.lua"].Name = "RCL_".._internal_randomstr(32)
Converted["_RCL.lua"].Parent = s_runservice:IsStudio() and s_players.LocalPlayer:WaitForChild("PlayerGui") or gethui() or s_coregui

Converted["_watermark"].Font = Enum.Font.RobotoMono
Converted["_watermark"].RichText = true
Converted["_watermark"].Text = "<font color=\"#31ff42\">RCL</font> | focat9123123 | 00:00:00"
...
Converted["_watermark"].Size = UDim2.new(0, 300, 0, 29)
Converted["_watermark"].Name = "watermark"
Converted["_watermark"].Parent = Converted["_RCL.lua"]
...
Converted["_notifs"].Parent = Converted["_RCL.lua"]
...
function notif_lib:MakeNotification(notif_table: table)
	...
	newNotif.Name = "RCL.lua - ".._internal_randomstr(8)
...
local function _watermark_tick()
	...
	env["_internal"]["update_watermark"] = function()
		...
		watermark_text.Text = string.format("<font color=\"#31ff42\">RCL</font> | %s | %s", displayname, time)
	end
	...
	print(string.format("[ RCL.lua ] [ %s ] tick service connected", env["_internal"]["get_time"]()))
end
...
function animate(obj, properties, duration, easingStyle, easingDirection)
	local tweenInfo = TweenInfo.new(duration, easingStyle or Enum.EasingStyle.Quad, easingDirection or Enum.EasingDirection.InOut)
	local tween = s_tweenservice:Create(obj, tweenInfo, properties)
	tween:Play()
	return tween
end
...
function Library:New(options: table)
	...
	do
		...
		GUI["2"]["Name"] = [[RCL window]];
	end
	
	--// Title bar (+ seperator)
	do
		-- StarterGui.ui lib.RCL window.sep
		...
		-- StarterGui.ui lib.RCL window.titlebar
		...
		--// starts with "RCL"?
		if string.sub(options["Name"], 1, 9) == "RCL" then
			--// Green "sense" in "RCL.lua"
			...
			--// replace "RCL" with "game<font color=\"#31ff42\">sense</font>"
			options["Name"] = string.gsub(options["Name"], "RCL", "game<font color=\"#31ff42\">sense</font>");
		end
		
		-- StarterGui.ui lib.RCL window.titlebar.RCL.lua
		...
		GUI["3c"]["Name"] = "RCL_".._internal_randomstr(32)
		GUI["3c"]["Position"] = UDim2.new(0.03947, 0, 0.20455, 0);
		GUI["3c"]["RichText"] = true


		-- StarterGui.ui lib.RCL window.titlebar.ImageLabel (Close button)
	...
	do
		GUI["3d"].MouseButton1Click:Connect(function()
			animate(GUI["2"], {BackgroundTransparency = 1}, 0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
			wait(0.5)
			GUI["2"].Visible = false
			Library:Notify({
		...
		s_uis.InputBegan:Connect(function(input, gpe)
			...
			if input.KeyCode == Enum.KeyCode.K then
				if not GUI["2"].Visible then
					GUI["2"].Visible = true
					animate(GUI["2"], {BackgroundTransparency = 0}, 0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
				end
			end
		end)
	end
	...
	--// Navigation
	do
		-- StarterGui.ui lib.RCL window.tab selector
		...
		-- StarterGui.ui lib.RCL window.tab selector.UIListLayout
	...
	function GUI:CreateTab(options: table)
		...
		--// Render
		do
			-- StarterGui.ui lib.RCL window.tab selector.tab button
			...
			-- StarterGui.ui lib.RCL window.tab selector.tab button.UICorner
			Tab["2b"] = Instance.new("UICorner", Tab["2a"]);
			Tab["2b"]["CornerRadius"] = UDim.new(0, 2);


			-- StarterGui.ui lib.RCL window.tab selector.tab button.UIStroke
			...
			-- StarterTab.ui lib.RCL window.tab
			...
			-- StarterTab.ui lib.RCL window.tab.content
			...
			-- StarterTab.ui lib.RCL window.tab.content.UIListLayout
		...
		function Tab:Button(options: table)
			...
			do
				------------- BUTTON
				-- StarterButton.ui lib.RCL window.tab.content.button
				...
				-- StarterButton.ui lib.RCL window.tab.content.button.click
				...
				-- StarterButton.ui lib.RCL window.tab.content.button.click.icon
				...
				-- StarterButton.ui lib.RCL window.tab.content.button.UICorner
				Button["b"] = Instance.new("UICorner", Button["8"]);
				Button["b"]["CornerRadius"] = UDim.new(0, 2);


				-- StarterButton.ui lib.RCL window.tab.content.button.UIStroke
			...
			function Button:SetText(text: string)
				assert(text, string.format("[ RCL ] [ button_%s ] text is required", options["Name"]))
				assert(type(text) == "string", string.format("[ RCL ] [ button_%s ] text must be a string", options["Name"]))
			...
			function Button:SetCallback(fn)
				...
				if fn ~= nil then
					assert(typeof(fn) == "function", string.format("[ RCL ] [ button_%s ] callback must be either nil or a function", options.Name))
				end
				options["Callback"] = fn
			end
			...
			do
				...
				Button["9"].MouseButton1Down:Connect(function()
					...
					animate(Button["8"], {BackgroundColor3 = Color3.fromRGB(39, 39, 39)}, 0.2)
					--// Callback handle
					...
					if not s then
						...
						warn(string.format("[ RCL.lua ] [ %s - %s ] Error on button callback: %s", options.Name, _internal_gettime(), r))
					end
				end)
				Button["9"].MouseButton1Up:Connect(function()
					...
					if Button.Hover then
						--// Hover state
						animate(Button["8"], {BackgroundColor3 = Color3.fromRGB(26, 26, 26)}, 0.2)
						animate(Button["a"], {ImageColor3 = Color3.fromRGB(50, 255, 67)}, 0.2)
						animate(Button["9"], {TextColor3 = Color3.fromRGB(50, 255, 67)}, 0.2)
					else
						--// Reset
						animate(Button["8"], {BackgroundColor3 = Color3.fromRGB(26, 26, 26)}, 0.2)
					end
		...
		function Tab:Label(options: table)
			...
			do
				------------- LABEL
				-- StarterLabel.ui lib.RCL window.tab.content.label
			...
			function Label:SetText(text: string)
				assert(text, string.format("[ RCL ] [ label_%s ] text is required", options["GUID"]))
				assert(type(text) == "string", string.format("[ RCL ] [ label_%s ] text must be a string", options["GUID"]))
		...
		function Tab:Slider(options: table)
			...
			do
				------------------- SLIDER
				-- StarterGui.ui lib.RCL window.tab.content.slider
				...
				-- StarterGui.ui lib.RCL window.tab.content.slider.click
				...
				-- StarterGui.ui lib.RCL window.tab.content.slider.click.value
				...
				-- StarterGui.ui lib.RCL window.tab.content.slider.UICorner
				Slider["28"] = Instance.new("UICorner", Slider["25"]);
				Slider["28"]["CornerRadius"] = UDim.new(0, 2);


				-- StarterGui.ui lib.RCL window.tab.content.slider.UIStroke
				...
				-- StarterGui.ui lib.RCL window.tab.content.slider.SliderBG
				...
				-- StarterGui.ui lib.RCL window.tab.content.slider.SliderBG.UICorner
				Slider["2b"] = Instance.new("UICorner", Slider["2a"]);



				-- StarterGui.ui lib.RCL window.tab.content.slider.SliderBG.Slider
				...
				-- StarterGui.ui lib.RCL window.tab.content.slider.SliderBG.Slider.UICorner
			...
			function Slider:SetValue(v: number)
				...
				if v == nil then --// setting via mouse/drag
					...
					assert(v >= options.Min and v <= options.Max, string.format("[ RCL ] [ slider_%s ] value must be between %s and %s", options.Name, options.Min, options.Max))
					assert(typeof(v) == "number", "[ RCL ] [ slider_"..options["Name"].." ] value must be a number")
		...
		function Tab:Toggle(options: table)
			...
			do
				------------- TOGGLE
				-- StarterToggle.ui lib.RCL window.tab.content.toggle
				...
				-- StarterToggle.ui lib.RCL window.tab.content.toggle.click
				...
				-- StarterToggle.ui lib.RCL window.tab.content.toggle.click.toggle
				...
				-- StarterToggle.ui lib.RCL window.tab.content.toggle.click.toggle.UICorner
				Toggle["1b"] = Instance.new("UICorner", Toggle["1a"]);
				Toggle["1b"]["CornerRadius"] = UDim.new(0, 2);


				-- StarterToggle.ui lib.RCL window.tab.content.toggle.UICorner
				Toggle["1c"] = Instance.new("UICorner", Toggle["18"]);
				Toggle["1c"]["CornerRadius"] = UDim.new(0, 2);


				-- StarterToggle.ui lib.RCL window.tab.content.toggle.UIStroke
		...
		function Tab:Textbox(options: table)
			...
			do
				------------- Textbox
				-- StarterTextbox.ui lib.RCL window.tab.content.Textbox
				...
				-- StarterTextbox.ui lib.RCL window.tab.content.Textbox.UICorner
				Textbox["13"] = Instance.new("UICorner", Textbox["12"]);
				Textbox["13"]["CornerRadius"] = UDim.new(0, 2);


				-- StarterTextbox.ui lib.RCL window.tab.content.Textbox.UIStroke
				...
				-- StarterTextbox.ui lib.RCL window.tab.content.Textbox.input
				...
				-- StarterTextbox.ui lib.RCL window.tab.content.Textbox.icon
			...
			function Textbox:SetValue(text: string)
				assert(text, string.format("[ RCL ] [ Textbox_%s ] text is required", options["Placeholder"]))
	...
	Library:Notify({
		Description = "RCL.lua has been loaded.",
		Duration = 5
	})
	return GUI
end
